...
arg_list* parse_callation(token_list& Token_list) {
	...
	while (!check_symbol::is(Token_list.this_(), token_symbol::type::PAREN_RIGHT)) {
		result.push_back(parse_expression(Token_list));

		if (check_symbol::is(Token_list.this_(), token_symbol::type::COMMA)) {
			Token_list.next();
		}
		else if (check_symbol::is(Token_list.this_(), token_symbol::type::PAREN_RIGHT)) {
			// pass
		}
		else {
...
expr* parse_suffix(token_list& Token_list, expr* item) {
	...
	while (true) {
		...
		if (check_symbol::is(Token_list.this_(), token_symbol::type::PAREN_LEFT)) {
			Token_list.next();
			auto caller = parse_callation(Token_list); // @index+1
			item = new expr_callation { item, caller, item->BEGIN, caller->END };
		}

		else if (check_symbol::is(Token_list.this_(), token_symbol::type::DOT)) {
			Token_list.next();
